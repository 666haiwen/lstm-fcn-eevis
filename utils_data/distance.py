...
def related(x, y, d_type, beta=1.5):
...
def probability_based(x, y):
    ...
    for col in range(shape[1]):
        ...
        # if x_std <= 1e-12:
        #     print("std == {} where x in {}, vector is {}".format(x_std, col, x[:, col]))
        # if y_std <= 1e-12:
        #     print("std == {} where y in {}, vector is {}".format(y_std, col, y[:, col]))
        v_sum = np.sum((x[:, col] - y[:, col])**2)
        res[col] = chi2.cdf(0, df) if v_sum == 0 else chi2.cdf(v_sum/(x_std + y_std), df)
    return res


def kullback_Liebler(x, y, s):
    """ calculate distance between matrix x and y by
