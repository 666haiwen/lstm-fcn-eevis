...
def _iterate_sparse_X(X, namelist, shape):
    """This little hack returns a densified row when iterating over a large
    hdf5 files, instead of constructing a sparse matrix for every row that is
    expensive.
    """
    row = np.zeros(shape)
    for i in namelist:
        X[i].read_direct(row)
        yield i, np.reshape(row, (shape[0]*shape[1]))
...
class _CFNode(object):
    ...
    def append_subcluster(self, subcluster, copy=False):
        ...
        if copy is True:
            self.subclusters_.append(_CFSubcluster(subcluster=subcluster))
        else:
    ...
    def insert_cf_subcluster(self, subcluster):
        ...
        if not self.subclusters_:
            self.append_subcluster(subcluster, copy=True)
            return False
...
class _CFSubcluster(object):
    ...
    """
    def __init__(self, linear_sum=None, subcluster=None):
        if subcluster is not None:
            self.n_samples_ = subcluster.n_samples_
            self.centroid_ = self.linear_sum_ = np.copy(subcluster.linear_sum_)
            self.squared_sum_ = subcluster.squared_sum_
            self.sq_norm_ = subcluster.sq_norm_
        elif linear_sum is None:
            self.n_samples_ = 0
...
class Birch(BaseEstimator, TransformerMixin, ClusterMixin):
    ...
    * Roberto Perdisci
    ...
    def __init__(self, threshold=0.5, branching_factor=50, n_clusters=3,
                 compute_labels=True, copy=True, namelist=None, shape=None):
        self.threshold = threshold
        self.branching_factor = branching_factor
    ...
    def _fit(self, X):
        ...
        n_features = self.shape[0]* self.shape[1]
        ...
        for name, sample in iter_func(X, self.namelist, self.shape):
            print(name)
    ...
    def _check_fit(self):
        ...
        if is_fitted and self.shape[0] * self.shape[1] != len(self.subcluster_centers_[0]):
            raise ValueError(
    ...
    def predict(self, X):
        ...
        self._check_fit()
        n_clusters = len(self.subcluster_centers_)
        reduced_distance = np.zeros((len(self.namelist), n_clusters))
        i = -1
        for name, sample in _iterate_sparse_X(X, self.namelist, self.shape):
            i += 1
            for j in range(n_clusters):
                reduced_distance[i][j] = np.dot(sample, self.subcluster_centers_[j])
            reduced_distance[i] *= -2
            reduced_distance[i] += self._subcluster_norms
    ...
    def _global_clustering(self, X=None):
        ...
        self._subcluster_norms = np.zeros(len(centroids))
        for i in range(len(centroids)):
            self._subcluster_norms[i] = np.dot(centroids[i], centroids[i])
